...
@section('content')
    <div class="mb-6">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-semibold text-gray-900">Edit Client Review</h1>
        </div>
    </div>

    <div class="bg-white border border-gray-200 rounded-xl shadow-sm">
        <form action="{{ route('client-reviews.update', $clientReview) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="reviewer_name" name="reviewer_name" value="{{ old('reviewer_name', $clientReview->reviewer_name) }}" required
                ...
                <div>
                    ...
                    <input type="text" id="reviewer_title" name="reviewer_title" value="{{ old('reviewer_title', $clientReview->reviewer_title) }}"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            ...
            <div>
                ...
                <textarea id="review_content" name="review_content" rows="4" required
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Enter the review content...">{{ old('review_content', $clientReview->review_content) }}</textarea>
            ...
            <div>
                ...
                <select id="rating" name="rating" required
                    ...
                    <option value="5" {{ old('rating', $clientReview->rating) == 5 ? 'selected' : '' }}>5 Stars - Excellent</option>
                    <option value="4" {{ old('rating', $clientReview->rating) == 4 ? 'selected' : '' }}>4 Stars - Very Good</option>
                    <option value="3" {{ old('rating', $clientReview->rating) == 3 ? 'selected' : '' }}>3 Stars - Good</option>
                    <option value="2" {{ old('rating', $clientReview->rating) == 2 ? 'selected' : '' }}>2 Stars - Fair</option>
                    <option value="1" {{ old('rating', $clientReview->rating) == 1 ? 'selected' : '' }}>1 Star - Poor</option>
                </select>
                @error('rating')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                @if($clientReview->avatar_path)
                    <div class="mb-3 flex items-center gap-4">
                        <img src="{{ asset('storage/' . $clientReview->avatar_path) }}" alt="{{ $clientReview->reviewer_name }}" class="h-20 w-20 object-cover rounded-full border border-gray-200">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $clientReview->sort_order) }}" min="0"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        placeholder="0">
                    @error('sort_order')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="flex items-center">
                    <div class="flex items-center h-5">
                        <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $clientReview->is_active) ? 'checked' : '' }}
