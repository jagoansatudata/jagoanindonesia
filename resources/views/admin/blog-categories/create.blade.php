...
@section('content')
    <div class="mb-6">
        ...
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">Create New News Category</h1>
            <p class="text-sm text-gray-600 mt-1">Add a new category for organizing news posts.</p>
        </div>
    </div>
            ...
            <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <input type="text" id="color-text" name="color-text" 
                                   ...
                                   value="{{ old('color', '#3B82F6') }}" placeholder="#000000">
                        </div>
                        @error('color')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        <div class="flex items-center gap-3">
                            <div id="preview-color" class="w-8 h-8 rounded-lg flex items-center justify-center" style="background-color: {{ old('color', '#3B82F6') }};">
                                <span id="preview-letter" class="text-white text-xs font-bold">{{ strtoupper(substr(old('name', 'Category'), 0, 1)) }}</span>
                            </div>
                            <div class="min-w-0">
                                <div id="preview-name" class="font-semibold text-gray-900">{{ old('name', 'Category Name') }}</div>
                                <div class="text-xs text-gray-500">{{ Str::slug(old('name', 'Category Name')) }}</div>
                            </div>
                        </div>
                    </div>
                </div>
